defmodule Fonts.Tables.Maxp do
  def parse_table(checksum, offset, length, state) do
    ...
    %{"Binary" => binary, "Parser" => _parser, "Font" => _font} = state
  ...
  def gen_table(0x00005000, checksum, offset, length, state) do
    ...
    table = %{
      "maxp" => %{
        ...
        "Table Version" => Fonts.Tables.Tables.parse_version_number(table_version),
    ...
    put_in(state, ["Font", "Tables"], tables)
  end

  def gen_table(_table_version, checksum, offset, length, state) do
    %{"Binary" => binary, "Parser" => _parser, "Font" => font} = state
